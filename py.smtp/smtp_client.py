...
default_port = 25
#default_files = "C:\\000.bin\\01.python\\05.files"
default_files = "C:\\02.dev\\03.go_project\\GoHttpd\\wwwroot\\files"
#default_files = "C:\\02.dev\\01.python\\testfiles"
...
#   mailServer.sendmail(gmail_user, to, msg.as_string())
    ...
    SaveToFile(msg, subject)


def SaveToFile(msg, text):
...
#  mail(toEmail, str('%06d_' % i)+name , "Test email : " + name, filenames[i], str(os.getpid()) + '_' + name)
  
  subjectText = str('%06d' % i) + '.' + now.strftime('%Y%m%d') + '.' + name
#  print(subjectText)
  mail(toEmail, subjectText , "Test email : " + name, filenames[i], str(os.getpid()) + '_' + name)
  print("subject : " + subjectText)
#  print(subjectText)# + ' - '+ name)
 #  print(str(now) + ' : ' +  filenames[i] + ' - '+ name[n-1])
  #  mail(toEmail, str(now) + " " + str('%06d' % i) , "Test email : " + name[n-1], filenames[i], str(os.getpid()) + '_' + name[n-1])
