        ...
        for shape in slide.shapes:
            ...
            if shape.shape_type == 13: # 13은 Picture 타입
                try:
                    ...
                    # 저장할 파일 경로 설정
#                    image_filename = f"{product}_{title[0]} Slide{slide_idx + 1:03d}_Img{image_count + 1:03d}.{image_ext}"
                    image_filename = f"{product}_{title[0]}_{file_image_count+1:04d}.Slide{slide_idx + 1:03d}_Img{image_count + 1:03d}.{image_ext}"
   #                 image_path = os.path.join(filename, image_filename)
                    image_path = os.path.join(title[0], image_filename)
...
search("C:\\0002.dev\\01.python\\py.getpptximg\\pptx")
